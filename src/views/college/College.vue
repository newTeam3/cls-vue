  ...
  <div>
    ...
    <el-form :inline="true" :model="formInline" class="user-search">
      <el-form-item label="搜索：">
        <el-input size="small" v-model="formInline.id"  placeholder="输入学院id"></el-input>
      </el-form-item>
      <el-form-item label="">
        <el-input size="small" v-model="formInline.name" placeholder="输入学院名称"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog" center>
      <el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
        <el-form-item label="学院名" prop="name">
          <el-input size="small"  auto-complete="off" v-model="editForm.name" placeholder="请输入学院名称"></el-input>
        </el-form-item>
...
<script>
    ...
    import { findAllCollege,addCollege,updateCollege,banCollege,banRows } from '@/api/College'
    export default {
        name: "College",
...
//导出的方法
        exportExcel() {
          require.ensure([], () => {
            ...
            const tHeader = ['ID', '学院名称', '备注','当前状态'];
            // 上面设置Excel的表格第一行的标题
            const filterVal = ['id', 'name', 'remark','status'];
            // 上面的index、nickName、name是tableData里对象的属性
            const list = this.tableData;  //把data里的tableData存到list
            const data = this.formatJson(filterVal, list);
            export_json_to_excel(tHeader, data, '学院列表excel');
          })
        },
