  ...
  <div>
    ...
    <el-form :inline="true" :model="formInline" class="user-search">
      <el-form-item label="搜索：">
        <el-input size="small" v-model="formInline.id"  placeholder="输入职务id"></el-input>
      </el-form-item>
      <el-form-item label="">
        <el-input size="small" v-model="formInline.name" placeholder="输入职务名称"></el-input>
      </el-form-item>
      <el-form-item>
...
<script>
  ...
  import { findAllPosition,addPosition,updatePosition,banPosition,banRows } from '@/api/Position'
  export default {
    name: "Position",
...
//导出的方法
      exportExcel() {
        require.ensure([], () => {
          ...
          const tHeader = ['ID', '职务名称', '备注','当前状态'];
          // 上面设置Excel的表格第一行的标题
          const filterVal = ['id', 'name', 'remark','status'];
          // 上面的index、nickName、name是tableData里对象的属性
          const list = this.tableData;  //把data里的tableData存到list
          const data = this.formatJson(filterVal, list);
          export_json_to_excel(tHeader, data, '职务列表excel');
        })
      },
