    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog" center>
      <el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
        <el-form-item label="角色名" prop="roleName">
          <el-input size="small"  auto-complete="off" v-model="editForm.roleName" placeholder="请输入角色名称"></el-input>
        </el-form-item>
...
<script>
  ...
  import { findAllRoles,addRole,updateRole } from '@/api/Role'
  export default {
    name: "Role",
    ...
    methods: {
      ...
      callFather(parm) {
        ...
        let para = {page: this.formInline.page,
          ...
          key:{id:this.formInline.id,roleName:this.formInline.roleName}};
        this.getData(para)
      },
        ...
        this.$refs.editForm.validate(valid => {
          if (valid) {
            this.$confirm("确认提交吗？", "提示", {}).then(() => {
              let para = {roleName:this.editForm.roleName,
...
//导出的方法
      exportExcel() {
        require.ensure([], () => {
          ...
          const tHeader = ['ID', '角色名称','备注'];
          // 上面设置Excel的表格第一行的标题
          const filterVal = ['id', 'roleName','remark'];
          // 上面的index、nickName、name是tableData里对象的属性
          const list = this.tableData;  //把data里的tableData存到list
          const data = this.formatJson(filterVal, list);
          export_json_to_excel(tHeader, data, '角色列表excel');
        })
      },
